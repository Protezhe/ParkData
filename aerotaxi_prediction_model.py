"""
Предсказательная модель для посещаемости Аэротакси.
Обучение на данных 2024 года, предсказание на 2025 год.
Сравнение предсказанных значений с реальными.

Оптимальный набор признаков (8 признаков, R2 = 0.692):
- is_weekend, is_holiday, is_rain, is_heavy_rain
- Темп_комфорт, Темп_диапазон
- День_1 (вторник), День_5 (суббота)
"""
import pandas as pd
import numpy as np
...
# Настройки
WINDOW_SIZE = 14  # Для скользящей медианы на графике
...
# Фильтруем только строки с данными Аэротакси
df = df[df['Аэротакси'].notna() & (df['Аэротакси'] > 0)].copy()

print(f"Всего записей с данными Аэротакси: {len(df)}")
...
# Производные температурные признаки
...
y_train = train_df['Аэротакси']

X_test = test_df[feature_cols].fillna(0)
y_test = test_df['Аэротакси']

# Обучение модели (параметры оптимизированы для предотвращения переобучения)
...
y_pred = np.maximum(y_pred, 0)  # Не допускаем отрицательных значений
...
# Важность признаков
print("\nВажность признаков (оптимальный набор):")
...
for _, row in feature_importance.iterrows():
    ...
    print(f"  {row['Признак']:<20} {row['Важность']:.3f} {bar}")
...
def plot_prediction_comparison():
    ...
    for j, (is_weekend, day_type_name) in enumerate(day_types):
        ...
        # Реальные данные 2025 (синяя линия с маркерами)
        days_real, median_real = calculate_rolling_median(test_df, 'Аэротакси', is_weekend)
        if len(days_real) > 0:
        ...
        # Предсказанные данные 2025 (красная линия с маркерами)
        ...
        ax_top.set_title(f'Аэротакси - {day_type_name}', fontsize=12, fontweight='bold')
        ...
        # Реальные данные 2024 (синяя линия)
        days_2024, median_2024 = calculate_rolling_median(train_df, 'Аэротакси', is_weekend)
        if len(days_2024) > 0:
        ...
        ax_bottom.set_title(f'Аэротакси 2024 (обучающие данные) - {day_type_name}',
    ...
    plt.suptitle('Предсказание посещаемости Аэротакси: модель vs реальность\n'
                 f'MAE = {mae:.1f} билетов, R2 = {r2:.3f}',
                 fontsize=14, fontweight='bold', y=0.98)
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])

    filename = 'aerotaxi_prediction_comparison.png'
...
print("-" * 60)
print(f"{'Месяц':<10} {'Реально':<12} {'Предсказано':<12} {'Разница':<12}")
print("-" * 60)

for month in sorted(test_df['Месяц'].unique()):
    month_data = test_df[test_df['Месяц'] == month]
    real_median = month_data['Аэротакси'].median()
    ...
    print(f"{month_names.get(month, month):<10} {real_median:<12.0f} {pred_median:<12.0f} {diff:+.0f}")
